...
MyGreenCar is the first server released by our group and provides a simple and easy-to-learn introduction to the prototype of our platform. The MyGreenCar server implements the HTTP API and includes a JS/jQuery web interface. The server is written in Python and the HTTP API is built upon a Flask app. To enable real-time communication, MyGreenCar also supports the addition of a WebSocket port using an Autobahn/Twisted server. An SQLite database is employed for storage. We have elected to leave the API exposed in the server, demo, and interface code to enable easy modification and experimentation.
...
The MyGreenCar server is capable of broadcasting changes to the network over a WebSocket port. This allows devices and interfaces to be updated in real-time by receiving all successful responses to create, update, and delete requests. Before enabling the WebSocket funtionality, you will need to install Autobahn|Python, an open-source server for the WebSocket protocol, which can be installed with pip
```sh
...
The MyGreenCar server is still in beta development, so if you find a bug, please let us know.

#### License

The MyGreenCar source code is licensed under the [AGPL v3][agpl]. You can find a reference to this license at the top of each source code file.
...
In summary, any modifications to the MyGreenCar source code must be distributed according to the terms of the AGPL v3. Any code that connects to a MyGreenCar server via an API is recognized as a seperate work (not a derivative work) irrespective of where it runs. Lastly, you are free to modify the HTML, JS, and CSS code of the web interface without restrictions, though we would appreciate you sharing what you have created.
